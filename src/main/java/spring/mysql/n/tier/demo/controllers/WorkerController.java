package spring.mysql.n.tier.demo.controllers;

import java.sql.SQLException;
import java.util.List;

import spring.mysql.n.tier.demo.models.Worker;
import spring.mysql.n.tier.demo.services.WorkerService;
...
public class WorkerController {
	...
	public String updateBySalary(String department, Integer bonusfactor) {
		try {
			if (Boolean.TRUE.equals(this.workerService.updateWorkerSalary(department, bonusfactor))) {
				return String.format("Record of workers belonging to" + department
						+ "department is updated successfully with " + bonusfactor + " as bonus");
			}
		} catch (SQLException ex) {
			System.out.println("Exception occurred while updating the record of worker #" + bonusfactor + "!\n" + ex);
		}

		return String.format("Failure in updating the record of worker #%d!", bonusfactor);
	}
}
